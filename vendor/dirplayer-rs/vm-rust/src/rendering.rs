...
pub fn render_preview_bitmap(
    ...
    match &member.member_type {
        ...
        CastMemberType::Font(font_member) => {
            ...
            let font: Rc<BitmapFont> = if let (Some(req_size), Some(ref raw), Some(ref parsed)) =
                (preview_font_size, &pfr_data, &pfr_parsed)
            {
...
pub fn compute_filmloop_initial_rect_with_members(
    ...
    for (_frame_idx, channel_idx, data) in film_loop.score.channel_initialization_data.iter() {
        ...
        let (actual_width, actual_height, reg_x, reg_y) = if let Some(sprite_member) = player
            .movie
            .cast_manager
            .find_member_by_ref(&sprite_member_ref)
        {
    ...
    for (_frame_idx, channel_idx, data) in sorted_data {
        ...
        match &member.member_type {
            CastMemberType::Bitmap(bitmap_member) => {
                ...
                let should_use_matte =
                    (is_indexed && (ink == 0 || ink == 8)) || (is_16bit && ink == 0);
    ...
    for channel_num in sorted_channel_numbers {
        ...
        match &member.member_type {
            ...
            CastMemberType::Text(text_member) => {
                ...
                if font_opt.is_none() && font_style.is_some() {
                    web_sys::console::log_1(
                        &format!("⚠️ Font not found with style, trying without style...").into(),
                    );
                    font_opt = get_or_load_font(
                        &mut player.font_manager,
                        &player.movie.cast_manager,
                ...
                if font_opt.is_none() {
                    web_sys::console::log_1(
                        &format!("⚠️ Font not found with size, trying without size...").into(),
                    );
                    font_opt = get_or_load_font(
                        &mut player.font_manager,
                        &player.movie.cast_manager,
...
fn create_canvas2d_renderer(canvas_size: (u32, u32)) -> PlayerCanvasRenderer {
        ...
        if let Some(renderer) = renderer_lock {
            match renderer {
                ...
                DynamicRenderer::WebGL2(webgl_renderer) => {
                    ...
                    container_element
                        .append_child(webgl_renderer.canvas())
                        .unwrap();
                }
            }
        }
