...
/// Pointer events from JS use the `#stage_canvas_container` CSS box (`stage_size`).
/// Sprites and hit-testing use movie stage coordinates (`movie.rect`). Without this
/// mapping, clicks miss when the host scales the stage (e.g. full-width player).
fn map_stage_pointer_to_movie_coords(x: f64, y: f64) -> (i32, i32) {
    reserve_player_ref(|player| {
        let (sw, sh) = player.stage_size;
        if sw == 0 || sh == 0 {
            return (x as i32, y as i32);
        }
        let mw = player.movie.rect.width() as f64;
        let mh = player.movie.rect.height() as f64;
        if mw <= 0.0 || mh <= 0.0 {
            return (x as i32, y as i32);
        }
        let mx = x * mw / sw as f64;
        let my = y * mh / sh as f64;
...
pub fn mouse_down(x: f64, y: f64) {
    let (mx, my) = map_stage_pointer_to_movie_coords(x, y);
    ...
    reserve_player_mut(|player| {
        player.mouse_loc = (mx, my);
        player.movie.mouse_down = true;
    });
    player_dispatch(PlayerVMCommand::MouseDown((mx, my)));
}
...
pub fn mouse_up(x: f64, y: f64) {
    ...
    player_dispatch(PlayerVMCommand::MouseUp((mx, my)));
}
...
pub fn mouse_move(x: f64, y: f64) {
    ...
    player_dispatch(PlayerVMCommand::MouseMove((mx, my)));
}
...
pub fn player_get_sprite_at(x: f64, y: f64) -> i32 {
    let (mx, my) = map_stage_pointer_to_movie_coords(x, y);
    reserve_player_ref(|player| {
        get_sprite_at(player, mx, my, false)
            .map(|n| n as i32)
            .unwrap_or(0)
    })
}
...
pub fn set_glyph_preference(mode: &str) {
    use player::font::{set_glyph_preference as set_pref, GlyphPreference};
...
pub fn get_glyph_preference() -> String {
    use player::font::{get_glyph_preference as get_pref, GlyphPreference};
    match get_pref() {
