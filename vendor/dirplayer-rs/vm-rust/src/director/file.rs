...
/// True when all four bytes of the codec u32 look like ASCII (common when HTML/text is misread as RIFX).
fn rifx_type_field_looks_like_printable_text(codec: u32) -> bool {
    for shift in [24, 16, 8, 0] {
        let b = ((codec >> shift) & 0xff) as u8;
        let ok = matches!(b, b'\t' | b'\n' | b'\r') || (0x20..=0x7e).contains(&b);
        if !ok {
            return false;
        }
    }
...
impl DirectorFile {
    ...
    fn read(
        ...
        if meta_fourcc != FOURCC("RIFX") && meta_fourcc != FOURCC("XFIR") {
            return Err(format!(
                "Not a Director RIFX container: expected RIFX or XFIR at offset 0, got '{}' ({:#010x}). \
If you uploaded a file, it may be HTML/JSON, the wrong asset, or not a Shockwave .dcr.",
                fourcc_to_string(meta_fourcc),
                meta_fourcc
            ));
        }
        ...
        // RIFX "type" at offset 8. ScummVM also treats MV93, MC95, APPL as mmap-based;
        // MV95 matches Director builds that use the same layout as MV93.
        if codec == FOURCC("MV93")
            || codec == FOURCC("MC95")
            || codec == FOURCC("MV95")
        {
            ...
            let mut msg = format!(
                "Unsupported Director RIFX type '{}' ({:#010x}). Expected one of: MV93, MC95, MV95, FGDM, FGDC.",
                fourcc_to_string(codec),
                codec
            );
            if rifx_type_field_looks_like_printable_text(codec) {
                msg.push_str(
                    " The type field looks like plain text (e.g. a fragment of “the”), so this is probably not a valid binary .dcr — file may be corrupted, truncated, or saved from a web page by mistake.",
                );
            }
            return Err(msg);
        }
...
fn parse_fmap_v4(data: &[u8]) -> HashMap<u16, String> {
    ...
    for i in 0..entries_used {
        ...
        let font_name =
            String::from_utf8_lossy(&data[name_data_start..name_data_start + name_length])
                .trim_end_matches('\0')
                .to_string();
...
fn parse_vwfm(data: &[u8]) -> HashMap<u16, String> {
    ...
    for (i, &font_id) in font_ids.iter().enumerate() {
        ...
        let font_name =
            String::from_utf8_lossy(&data[current_pos..current_pos + name_len]).to_string();
...
fn parse_font_table(
    ...
    let chunk_ids: Vec<(u32, u32)> = chunk_container
        .chunk_info
        .iter()
...
pub fn get_script_context_chunk(
    ...
    let chunk = match get_chunk(reader, chunk_container, rifx, fourcc, section_id) {
        ...
        Err(e) => {
            console_warn!(
                "get_script_context_chunk: Could not find chunk {} ${:x}: {}",
    ...
    if let Chunk::ScriptContext(context) = chunk {
        ...
        console_warn!(
            "get_script_context_chunk: Chunk {} ${:x} is not a ScriptContext",
            fourcc_to_string(fourcc),
            section_id
        );
        return None;
...
pub fn get_script_names_chunk(
    ...
    let chunk = match get_chunk(reader, chunk_container, rifx, fourcc, section_id) {
        ...
        Err(e) => {
            console_warn!(
                "get_script_names_chunk: Could not find chunk {} ${:x}: {}",
                fourcc_to_string(fourcc),
                section_id,
                e
            );
    ...
    if let Chunk::ScriptNames(names) = chunk {
        ...
        console_warn!(
            "get_script_names_chunk: Chunk {} ${:x} is not a ScriptNames",
            fourcc_to_string(fourcc),
            section_id
        );
        return None;
        ...
        Some(info) => {
            ...
            if chunk_container.cached_chunk_views.contains_key(&id) {
                ...
                if info.len == 0 && info.uncompressed_len == 0 {
                    chunk_container.cached_chunk_views.insert(
                        id,
                        reader
                            .read_bytes(info.len)
                            .map_err(|e| {
                                format!("Chunk {}: failed to read empty chunk: {}", id, e)
                            })?
                            .to_vec(),
                    );
                    ...
                    {
                        uncomp_buf = Some(reader.read_zlib_bytes(info.len).map_err(|e| {
                            format!("Chunk {}: zlib decompression failed: {}", id, e)
                        })?);
                ...
                chunk_container.cached_chunk_views.insert(id, chunk_data);
            }
