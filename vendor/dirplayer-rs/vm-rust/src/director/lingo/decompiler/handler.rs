...
impl<'a> DecompilerState<'a> {
    ...
    fn add_statement(&mut self, node: Rc<AstNode>, bytecode_indices: Vec<usize>) {
        self.current_block
            .borrow_mut()
            .add_child(node, bytecode_indices);
    }
        ...
        for start_index in 0..bytecode_array.len() {
            ...
            match loop_type {
                ...
                BytecodeTag::RepeatWithTo | BytecodeTag::RepeatWithDownTo => {
                    let end_repeat = &bytecode_array[end_index - 1];
                    if let Some(&condition_start_index) = self
                        .bytecode_pos_map
                        .get(&(end_repeat.pos - end_repeat.obj as usize))
                    {
    ...
    fn get_var_name_from_set(&self, index: usize) -> String {
        ...
        match bytecode.opcode {
            OpCode::SetGlobal | OpCode::SetGlobal2 | OpCode::SetProp => self.get_name(bytecode.obj),
                                ...
                                CaseExpect::Otherwise => {
                                    ...
                                    for child in children.iter().rev() {
                                        ...
                                        } = child.node.as_ref()
                                        {
                                            ...
                                            if ow_pos >= 0 {
                                                if let Some(&ow_index) =
                                                    self.bytecode_pos_map.get(&(ow_pos as usize))
                                                {
                                                    self.bytecode_tags[ow_index].tag =
                                                        BytecodeTag::EndCase;
                                                }
                                            }
                                            self.enter_block(
                                                ow.borrow().block.clone(),
                                                BlockContext::CaseOtherwise,
                                            );
                                            break;
                                        }
                                    }
    ...
    fn translate_bytecode(&mut self, index: usize) -> usize {
        ...
        let translation: Option<Rc<AstNode>> = match opcode {
            ...
            OpCode::PushZero => Some(Rc::new(AstNode::Literal(Datum::int(0)))),
            ...
            OpCode::PushSymb => Some(Rc::new(AstNode::Literal(Datum::symbol(self.get_name(obj))))),

            OpCode::PushVarRef => Some(Rc::new(AstNode::Literal(Datum::var_ref(
                self.get_name(obj),
            )))),
            ...
            OpCode::GetProp => Some(Rc::new(AstNode::Var(self.get_name(obj)))),

            OpCode::GetParam => Some(Rc::new(AstNode::Var(self.get_argument_name(obj)))),

            OpCode::GetLocal => Some(Rc::new(AstNode::Var(self.get_local_name(obj)))),
            ...
            OpCode::SetGlobal | OpCode::SetGlobal2 => {
                ...
                Some(Rc::new(AstNode::Assignment {
                    variable: var,
                    value,
                    force_verbose: false,
                }))
            ...
            OpCode::Jmp => self.translate_jmp(index, obj, &mut next_block),

            OpCode::EndRepeat => {
                // Should normally be tagged and skipped
                Some(Rc::new(AstNode::Comment(
                    "ERROR: Stray endrepeat".to_string(),
                )))
            }

            OpCode::JmpIfZ => self.translate_jmpifz_with_indices(
                index,
                obj,
                &mut next_block,
            ...
            OpCode::LocalCall => {
                ...
                let handler_name = if (obj as usize) < self.chunk.handlers.len() {
                    ...
                    self.lctx
                        .names
                        .get(handler_def.name_id as usize)
                        .cloned()
                        .unwrap_or_else(|| format!("handler_{}", obj))
                } else {
                    format!("handler_{}", obj)
                };
                Some(Rc::new(AstNode::Call {
                    name: handler_name,
                    args: arg_list,
                }))
            }
            ...
            OpCode::GetMovieProp => Some(Rc::new(AstNode::The(self.get_name(obj)))),
            ...
            OpCode::GetObjProp | OpCode::GetChainedProp => {
                ...
                Some(Rc::new(AstNode::ObjProp {
                    obj: object,
                    prop: self.get_name(obj),
                }))
            }
            ...
            OpCode::SetObjProp => {
                let value = self.pop_with_indices(&mut collected_indices);
                let object = self.pop_with_indices(&mut collected_indices);
                let prop = Rc::new(AstNode::ObjProp {
                    obj: object,
                    prop: self.get_name(obj),
                });
                Some(Rc::new(AstNode::Assignment {
            ...
            OpCode::GetTopLevelProp => Some(Rc::new(AstNode::Var(self.get_name(obj)))),

            OpCode::NewObj => {
                let obj_args = self.pop_with_indices(&mut collected_indices);
                Some(Rc::new(AstNode::NewObj {
                    obj_type: self.get_name(obj),
                    args: obj_args,
                }))
            }
    ...
    fn translate_jmp(
        ...
        index: usize,
        obj: i64,
        next_block: &mut Option<(Rc<RefCell<BlockNode>>, BlockContext)>,
    ) -> Option<Rc<AstNode>> {
        let bytecode_array = &self.handler.bytecode_array;
        ...
        let target_index = match self.bytecode_pos_map.get(&target_pos) {
            Some(&idx) => idx,
            None => {
                return Some(Rc::new(AstNode::Comment(
        ...
        Some(Rc::new(AstNode::Comment(
            "ERROR: Could not identify jmp".to_string(),
        )))
    }

    fn translate_jmpifz_with_indices(
        &mut self,
        index: usize,
        obj: i64,
        next_block: &mut Option<(Rc<RefCell<BlockNode>>, BlockContext)>,
        ...
        match tag {
            ...
            BytecodeTag::RepeatWithTo | BytecodeTag::RepeatWithDownTo => {
                ...
                let condition_start_index = self
                    .bytecode_pos_map
                    .get(&condition_start_pos)
                    .copied()
                    .unwrap_or(0);
                ...
                "count" if nargs == 2 => {
                    if let Some(datum) = args[1].get_value() {
                        if datum.datum_type == DatumType::Symbol {
                            let obj = args[0].clone();
                            let prop_name = datum.string_value.clone();
                            let prop_expr = Rc::new(AstNode::ObjProp {
                                obj,
                                prop: prop_name,
                            });
                            return Some(Rc::new(AstNode::ObjProp {
                                obj: prop_expr,
    ...
    fn read_chunk_ref_with_indices(
        &mut self,
        string: Rc<AstNode>,
        indices: &mut Vec<usize>,
    ) -> Rc<AstNode> {
        ...
        match property_type {
            0x00 => {
                if property_id <= 0x0b {
                    ...
                    Some(Rc::new(AstNode::LastStringChunk {
                        chunk_type,
                        obj: string,
                    }))
                }
            ...
            0x01 => {
                ...
                Some(Rc::new(AstNode::StringChunkCount {
                    chunk_type,
                    obj: string,
                }))
            }
            0x02 => {
                ...
                Some(Rc::new(AstNode::MenuProp {
                    menu_id,
                    prop: property_id as u32,
                }))
            }
            0x03 => {
                // Menu item property
                ...
                Some(Rc::new(AstNode::MenuItemProp {
                    menu_id,
                    item_id,
                    prop: property_id as u32,
                }))
            ...
            0x05 => {
                ...
                Some(Rc::new(AstNode::Comment(
                    "ERROR: Resource property".to_string(),
                )))
            }
            0x06 => {
                // Sprite property
                let sprite_id = self.pop();
                Some(Rc::new(AstNode::SpriteProp {
                    sprite_id,
                    prop: property_id as u32,
                }))
            }
            ...
            0x08 => {
                ...
                if property_id == 0x02 && self.version >= 500 {
                    ...
                    if !is_zero {
                        ...
                        return Some(Rc::new(AstNode::TheProp {
                            obj: cast_lib_node,
                            prop: prop_name,
                        }));
                    }
            ...
            0x09..=0x15 => {
                ...
                Some(Rc::new(AstNode::TheProp {
                    obj: entity,
                    prop: prop_name,
                }))
            }
            _ => Some(Rc::new(AstNode::Comment(format!(
    ...
    fn generate_output(&self) -> DecompiledHandler {
        let name = self
            .lctx
            .names
            .get(self.handler.name_id as usize)
            .cloned()
        ...
        Self::collect_block_lines(
            &self.root_block.borrow(),
            dot,
            0,
            &mut lines,
            &mut bytecode_to_line,
        );
    ...
    fn push_line(
        ...
        indices: Vec<usize>,
        indent: u32,
        lines: &mut Vec<DecompiledLine>,
        bytecode_to_line: &mut HashMap<usize, usize>,
    ) {
    ...
    fn collect_block_lines(
        ...
        for child in &block.children {
            Self::collect_statement_lines(
                &child.node,
                &child.bytecode_indices,
                dot,
                indent,
                lines,
                bytecode_to_line,
