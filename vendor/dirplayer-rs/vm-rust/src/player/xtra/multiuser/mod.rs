...
use crate::{
    ...
    player::{
        self, events::player_dispatch_callback_event, reserve_player_mut, reserve_player_ref,
    ...
    pub fn call_instance_handler(
        ...
        match handler_name.to_lowercase().as_str() {
            ...
            "connecttonetserver" => {
                ...
                multiuser_log!(
                    "Multiuser: Connecting to WebSocket URL: {} (user={}, movie={})",
                    ws_url,
                    username,
                    movie_id
                );
                    ...
                    let message_str =
                        if let Ok(array_buffer) = data.clone().dyn_into::<js_sys::ArrayBuffer>() {
                            ...
                            let char_vec: Vec<char> =
                                vec.into_iter().map(|byte| byte as char).collect();
