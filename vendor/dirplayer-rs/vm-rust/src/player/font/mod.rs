...
pub fn set_glyph_preference(pref: GlyphPreference) {
    ...
    web_sys::console::log_1(&format!("[GlyphPreference] Set to {:?}", pref).into());
}
            ...
            if let Some(key) = matched_key {
                if let Some(pfr_bytes) = self.default_pfr_data.get(&key) {
                    ...
                    match parse_pfr1_font_with_target(pfr_bytes, 0) {
                        ...
                        Err(e) => {
                            web_sys::console::log_1(
                                &format!("Failed to parse default PFR font '{}': {}", key, e)
                                    .into(),
                            );
                        }
                    }
    ...
    for raw in &raw_lines {
        ...
        if word_wrap && max_width > 0 {
            ...
            for word in raw.split_whitespace() {
                let word_width: u16 = word.chars().map(|c| font.get_char_advance(c as u8)).sum();
            ...
            let line_width: u16 = raw.chars().map(|c| font.get_char_advance(c as u8)).sum();
...
pub fn get_text_char_pos(text: &str, params: &DrawTextParams, char_index: usize) -> (i16, i16) {
    ...
    for c in text.chars() {
        if c == '\r' || c == '\n' {
            ...
            y += params.line_height.unwrap_or(eff_lh) as i16 + params.line_spacing as i16 + 1;
        } else {
            if line_index == char_index {
                return (x, y);
            }
