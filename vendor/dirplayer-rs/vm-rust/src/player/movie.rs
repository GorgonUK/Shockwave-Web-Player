...
use super::{
    ...
    geometry::IntRect, net_manager::NetManager, reserve_player_mut, score::Score, ScriptError,
    ScriptReceiver,
};
...
impl Movie {
    ...
    pub fn get_prop(&self, prop: &str) -> Result<Datum, ScriptError> {
        match prop {
            ...
            "mouseDownScript" | "mouseUpScript" | "keyDownScript" | "keyUpScript"
            | "timeoutScript" => {
            ...
            "rollOver" => reserve_player_ref(|player| {
                let sprite = super::score::get_sprite_at(
                    player,
                    player.mouse_loc.0,
                    player.mouse_loc.1,
                    false,
    ...
    pub fn set_prop(
        ...
        match prop {
            ...
            | "timeoutScript" => {
                ...
                return match value {
                    ...
                    _ => Err(ScriptError::new(format!(
                        "String, object or 0 expected for {} value",
                        prop
                    ))),
                };
            }
            ...
            | "timeoutLapsed" | "soundEnabled" | "soundLevel" | "beepOn" | "centerStage"
            | "exitLock" | "fixStageSize" | "stageColor" => {
                // Anim props that are set via property_type 0x07 - accept silently
            }
            "randomSeed" => {
                self.random_seed = Some(value.int_value()?);
            }
            "stageColor" => match value {
                ...
                _ => {
                    return Err(ScriptError::new(
                        "Integer color index expected for stageColor".to_string(),
                    ));
                }
            },
