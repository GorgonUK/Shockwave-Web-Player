...
impl DirPlayer {
    ...
    pub async fn load_movie_from_file(&mut self, path: &str) -> Result<(), ScriptError> {
    ...
    pub fn begin_all_sprites(&mut self) {
        self.movie
            .score
            .begin_sprites(ScoreRef::Stage, self.movie.current_frame);
        ...
        let mut processed_film_loops: std::collections::HashSet<CastMemberRef> =
        ...
        for channel in self.movie.score.channels.iter() {
            ...
            match member_type {
                Some(CastMemberTypeId::FilmLoop) => {
                    ...
                    film_loop
                        .score
                        .begin_sprites(ScoreRef::FilmLoop(member_ref_clone), current_frame);
            ...
            match member_type {
                Some(CastMemberTypeId::FilmLoop) => {
                    ...
                    all_ended_sprite_nums
                        .extend(ended_sprite_nums.iter().map(|&x| (score_ref.clone(), x)));
                }
                _ => {}
            }
        }
    ...
    pub fn initialize_globals(&mut self) {
        ...
        self.globals
            .insert("BACKSPACE".to_string(), backspace_datum);
        ...
        for (cid, entry) in self.allocator.datums.iter() {
            ...
            match &entry.datum {
                ...
                Datum::Point(arr) => {
                    ...
                    if nc > 0 {
                        growing_containers
                            .push((format!("OLD Point #{} (rc={}) +{} new", cid, rc, nc), nc));
                        for r in arr {
                ...
                Datum::Rect(arr) => {
                    ...
                    if nc > 0 {
                        growing_containers
                            .push((format!("OLD Rect #{} (rc={}) +{} new", cid, rc, nc), nc));
                        for r in arr {
                            if let Some(id) = ref_id(r).filter(|id| new_datum_ids.contains(id)) {
                                accounted.insert(id);
                            }
            ...
            for (prop_name, r) in &entry.script_instance.properties {
                if let Some(id) = ref_id(r).filter(|id| new_datum_ids.contains(id)) {
                    ...
                    let dtype = self
                        ...
                        .unwrap_or("?".to_string());
                    *si_new
                        .entry(format!("si({}).{} [{}]", sn, prop_name.as_str(), dtype))
                        .or_insert(0) += 1;
                    accounted.insert(id);
                }
            }
        ...
        match prop {
            ...
            "currentSpriteNum" => {
                ...
                if let Some(script_instance_ref) = script_instance_ref {
                    ...
                    if let Some(datum_ref) =
                        script_get_prop_opt(self, &script_instance_ref, &"spriteNum".to_owned())
                    {
    ...
    pub fn push_scope(&mut self) -> ScopeRef {
        if (self.scope_count + 1) as usize >= MAX_STACK_SIZE {
            ...
            let start = if self.scope_count > 10 {
                self.scope_count - 10
            } else {
                0
            };
                ...
                if let Some(scope) = self.scopes.get(i as ScopeRef) {
                        ...
                        self.movie.cast_manager.get_script_by_ref(&scope.script_ref)
                    {
                        ...
                        let handler_name = script
                            ...
                            .unwrap_or_else(|| {
                                format!("handler_name_id#{}", scope.handler_name_id)
                            });
...
pub async fn player_call_script_handler_raw_args(
    ...
    let scope_generation =
        reserve_player_ref(|player| player.scopes.get(scope_ref).unwrap().generation);
    ...
    loop {
        ...
        if debugger_active {
            ...
            let should_step_break = reserve_player_ref(|player| match &player.step_mode {
                StepMode::None => false,
                StepMode::Into => true,
                StepMode::IntoLine {
                    skip_bytecode_indices,
        ...
        let result = match player_execute_bytecode(&ctx).await {
            ...
            Err(err) => {
                ...
                if err.code != ScriptErrorCode::Abort {
                    ...
                    if should_break {
                        let (current_script_ref, current_bytecode_idx) =
                            reserve_player_ref(|player| {
...
async fn run_movie_init_sequence() {
    ...
    for (behavior_ref, sprite_num) in &behaviors_to_init {
        if let Err(err) =
            Score::initialize_behavior_defaults_async(behavior_ref.clone(), *sprite_num).await
        {
    ...
    for behavior_ref in &remaining_behaviors {
        ...
        let _ = player_invoke_targeted_event(&"beginSprite".to_string(), &vec![], Some(&receivers))
            .await;
    }
...
pub async fn run_frame_loop() {
    ...
    while is_playing {
        ...
        if !is_script_paused {
            ...
            let skip_frame = reserve_player_ref(|player| {
                player.command_handler_yielding || player.in_mouse_command
            });
            if !skip_frame {
        ...
        if should_advance_frame && !is_script_paused && !is_delayed && !skip_frame {
            ...
            if has_player_frame_changed {
                ...
                if has_frame_changed_in_go && go_direction == 1 {
                    ...
                    if let Err(err) =
                        player_invoke_frame_and_movie_scripts(&"exitFrame".to_string(), &vec![])
                            .await
                    {
                        if err.code != ScriptErrorCode::Abort {
                            reserve_player_mut(|player| player.on_script_error(&err));
