...
use crate::{
    ...
    player::{
        datum_formatting::{datum_to_string_for_concat, format_datum},
        datum_ref::DatumRef,
    },
};
...
pub fn multiply_datums(
    ...
    let result = match (&left, &right) {
        (Datum::Void, Datum::Int(_)) | (Datum::Int(_), Datum::Void) => Datum::Int(0),
        (Datum::Void, Datum::Float(_)) | (Datum::Float(_), Datum::Void) => Datum::Float(0.0),
        ...
        (Datum::Rect(a), Datum::Int(right)) => {
            ...
            for i in 0..4 {
                ...
                let prod =
                    multiply_datums(player.alloc_datum(a_val), player.alloc_datum(b_val), player)?;
    ...
    let result = match (&left, &right) {
        ...
        (Datum::Point(a), Datum::Int(right)) => {
            ...
            for i in 0..2 {
                ...
                let b_val = player.get_datum(&right_ref).clone();
                let quot =
                    divide_datums(player.alloc_datum(a_val), player.alloc_datum(b_val), player)?;
