...
use super::{
    allocator::ScriptInstanceAllocatorTrait, cast_lib::CastMemberRef,
    handlers::datum_handlers::script_instance::ScriptInstanceUtils, player_call_script_handler,
    player_semaphone, reserve_player_ref, score::ScoreRef, script::ScriptInstanceId,
...
pub fn player_dispatch_event_to_sprite(
    ...
    let instance_ids = reserve_player_ref(|player| {
        ...
        if let Some(cached_ref) = player
            .script_instance_list_cache
            .get(&(sprite_num as i16))
            .cloned()
        {
...
pub async fn player_dispatch_event_to_sprite_targeted(
    ...
    for instance_id in instance_ids {
        player_invoke_targeted_event(handler_name, args, Some(&vec![instance_id].as_ref())).await;
    }
}
        ...
        match player_call_script_handler(Some(script_instance_ref), handler_ref, args).await {
            ...
            Err(err) => {
                if err.code != ScriptErrorCode::Abort {
                    ...
                    crate::player::bytecode::handler_manager::dump_execution_history_on_error(
                        &err.message,
                    );
...
pub async fn player_dispatch_movie_callback(handler_name: &str) -> Result<(), ScriptError> {
    enum CallbackAction {
    ...
    let action = reserve_player_ref(|player| {
        ...
        match callback {
            ScriptReceiver::ScriptInstance(instance_ref) => {
                ...
                Some(CallbackAction::CallHandler(
...
pub async fn run_event_loop(rx: Receiver<PlayerVMEvent>) {
    ...
    while !rx.is_closed() {
        ...
        let skip = reserve_player_ref(|player| {
            player.in_mouse_command || player.command_handler_yielding || player.is_in_transition
        });
        if skip {
            continue;
        }
...
pub async fn player_dispatch_event_beginsprite(
    ...
    let (mut sprite_instances, mut frame_instances, all_channels) = reserve_player_mut(|player| {
        ...
        let filtered_channels: Vec<_> = player
            .movie
            .score
            .channels
            .iter()
        ...
        for channel in filtered_channels {
            ...
            if channel.number == 0 {
                ...
                frame_instances.extend(instances.into_iter().map(|inst| (channel.number, inst)));
            } else {
                // Sprite behaviors (channel > 0) - include ScoreRef::Stage
                sprite_instances.extend(
                    instances
                        .into_iter()
                        .map(|inst| (ScoreRef::Stage, channel.number, inst)),
                );
            }
        ...
        for (member_ref, filmloop_score) in active_filmloops {
            ...
            let active_filmloop_channels: HashSet<u32> = filmloop_score
                .sprite_spans
                .iter()
            ...
            let filtered_filmloop_channels: Vec<_> = filmloop_score
                .channels
                .iter()
                .filter(|channel| !channel.sprite.script_instance_list.is_empty())
                .filter(|channel| channel.sprite.entered)
            ...
            for channel in filtered_filmloop_channels {
                ...
                if channel.number > 0 {
                    ...
                    sprite_instances.extend(
                        instances
                            .into_iter()
                            .map(|inst| (score_ref.clone(), channel.number, inst)),
                    );
                    all_channels.push((
    ...
    for (score_ref, sprite_number, behavior) in sprite_instances {
        ...
        if let Err(err) =
            player_invoke_targeted_event(handler_name, args, Some(receivers).as_ref()).await
        {
            if err.code == ScriptErrorCode::Abort {
                reserve_player_mut(|player| {
...
pub async fn dispatch_event_endsprite_for_score(score_ref: ScoreRef, sprite_nums: Vec<u32>) {
    let (sprite_tuple, frame_tuple) = reserve_player_mut(|player| {
    ...
    for (sprite_num, behaviors) in sprite_tuple {
        for behavior in behaviors {
            ...
            if let Err(err) =
                player_invoke_event_to_instances(&"endSprite".to_string(), &vec![], &receivers)
                    .await
            {
                if err.code == ScriptErrorCode::Abort {
                    break;
                ...
                web_sys::console::error_1(
                    &format!(
                        "Error in endSprite for sprite {}: {}",
                        sprite_num, err.message
                    )
                    .into(),
                );
                reserve_player_mut(|player| {
...
pub async fn dispatch_event_to_all_behaviors(handler_name: &String, args: &Vec<DatumRef>) {
    ...
    let skip = reserve_player_mut(|player| {
        ...
        if player.is_dispatching_events {
            warn!("Blocking re-entrant event dispatch for '{}'", handler_name);
            return true;
        }
...
pub async fn dispatch_system_event_to_timeouts(handler_name: &String, args: &Vec<DatumRef>) {
