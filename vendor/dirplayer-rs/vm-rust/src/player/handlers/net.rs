    ...
    pub fn get_stream_status(args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        reserve_player_mut(|player| {
            let (state, error, url, is_ok) = {
                    ...
                    match arg {
                        ...
                        _ => {
                            return Err(ScriptError::new(
                                "getStreamStatus requires an integer task ID or URL string"
                                    .to_string(),
                            ))
                        }
                    }
                ...
                let task = player.net_manager.get_task(task_id).ok_or_else(|| {
                    ScriptError::new(format!("Network task {} not found", task_id))
                })?;
