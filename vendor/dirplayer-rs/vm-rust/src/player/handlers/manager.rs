...
use crate::{
    ...
    player::{
        ...
        keyboard_map, player_alloc_datum, player_call_global_handler, player_call_script_handler,
...
const SWP_RUNTIME_TRACE_PREFIX: &str = "[ShockwaveWebPlayerRuntime]";
const SWP_RUNTIME_TRACE_LIMIT: usize = 180;
...
impl BuiltInHandlerManager {
    fn trace_value(value: &str) -> String {
        value
            ...
            .trim()
            .chars()
            .take(SWP_RUNTIME_TRACE_LIMIT)
            .collect()
    }

    fn emit_runtime_trace(parts: Vec<(&str, String)>) {
        let payload = parts
            .into_iter()
            .map(|(key, value)| format!("{key}={value}"))
            .collect::<Vec<_>>()
            .join("; ");
        JsApi::dispatch_debug_message(&format!("{SWP_RUNTIME_TRACE_PREFIX} {payload}"));
    }

    fn describe_args_for_trace(args: &Vec<DatumRef>) -> Result<(String, String), ScriptError> {
        reserve_player_ref(|player| {
            let arg_types = args
                .iter()
                .map(|arg| player.get_datum(arg).type_str().to_lowercase())
                .collect::<Vec<_>>()
                .join("|");
            let arg_values = args
                .iter()
                .map(|arg| {
                    Self::trace_value(&format_concrete_datum(&player.get_datum(arg), player))
                })
                .collect::<Vec<_>>()
                .join(" || ");
    ...
    fn trace_missing_builtin_fallback(
        handler_name: &String,
        args: &Vec<DatumRef>,
    ) -> Result<(), ScriptError> {
        let (arg_types, arg_values) = Self::describe_args_for_trace(args)?;
        Self::emit_runtime_trace(vec![
            ("kind", "missing_builtin_fallback".to_string()),
    ...
    fn get_pos(args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        if args.len() < 2 {
            return Err(ScriptError::new(
                "getPos requires 2 arguments (list, item)".to_string(),
            ));
        }

        let (arg_types, arg_values) = Self::describe_args_for_trace(args)?;
        Self::emit_runtime_trace(vec![
            ("kind", "getpos_patch_invoked".to_string()),
            ("handler", "getPos".to_string()),
            ("argTypes", arg_types),
            ("args", arg_values),
        ]);

        let list_ref = args[0].clone();
        let search_args = args[1..].to_vec();
        let result = reserve_player_mut(|player| match player.get_datum(&list_ref) {
            Datum::List(..) => ListDatumHandlers::find_pos(&list_ref, &search_args),
            Datum::PropList(..) => PropListDatumHandlers::get_pos(&list_ref, &search_args),
            _ => Err(ScriptError::new("Cannot getPos on non-list".to_string())),
        })?;

        let (return_type, return_value) = reserve_player_ref(|player| {
            let datum = player.get_datum(&result);
            Ok((
                datum.type_str().to_lowercase(),
                Self::trace_value(&format_concrete_datum(datum, player)),
            ))
        })?;

        Self::emit_runtime_trace(vec![
            ("kind", "getpos_patch_result".to_string()),
            ("handler", "getPos".to_string()),
                        ...
                        args_str
                            ...
                            .map(|arg| {
                                ...
                                if let Ok(i) = arg.parse::<i32>() {
                                    ...
                                    player.alloc_datum(Datum::String(
    ...
    pub fn call_handler(name: &String, args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        match name.as_str().to_lowercase().as_str() {
            ...
            _ => {
                ...
                let _ = Self::trace_missing_builtin_fallback(name, args);
                warn!("{msg}");
                return Err(ScriptError::new(msg));
            }
        }
