...
use crate::{
    ...
    player::{
        ...
        events::{
            ...
            player_invoke_global_event, player_invoke_static_event, player_invoke_targeted_event,
            player_unwrap_result, player_wait_available,
        },
        get_score_sprite_mut,
        handlers::datum_handlers::player_call_datum_handler,
        reserve_player_mut, reserve_player_mut_async, reserve_player_ref,
    ...
    pub async fn send_all_sprites(args: &Vec<DatumRef>) -> Result<DatumRef, ScriptError> {
        ...
        let skip = reserve_player_mut(|player| {
            if player.is_in_send_all_sprites {
                warn!("Blocking re-entrant sendAllSprites call to prevent infinite recursion");
                return true;
            }
