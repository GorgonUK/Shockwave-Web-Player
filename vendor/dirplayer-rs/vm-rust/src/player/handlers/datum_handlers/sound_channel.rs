...
impl SoundChannel {
    ...
    async fn play_current_segment_async(channel_rc: Rc<RefCell<Self>>) {
        ...
        web_sys::console::log_1(
            &"🚀 About to spawn MP3 decode task (play_current_segment_async)".into(),
        );
        ...
        if let Ok(member_name) = datum.string_value() {
                ...
                .find_member_ref_by_name(&member_name)
            {
                ...
                match &cast_member.member_type {
                    ...
                    _ => {
                        console::log_1(
                            &format!("⚠️ Member '{}' is not a sound", member_name).into(),
                        );
                        return None;
                    }
                }
    ...
    pub fn start_sound(self_rc: Rc<RefCell<Self>>, member_ref: DatumRef) {
        let (channel_num, audio_context, loop_count) = {
            ...
            if state == web_sys::AudioContextState::Suspended {
                ...
                console::log_1(
                    &format!("🎵 AudioContext resume result: {:?}", resume_result).into(),
                );
            }
        ...
        if let Some(sound_member) = Self::resolve_sound_member(player, &datum) {
            ...
            if let Some(mp3_bytes) = &audio_data.compressed_data {
                ...
                wasm_bindgen_futures::spawn_local(async move {
                    ...
                    if let Err(e) =
                        Self::start_sound_mp3_async(self_rc_clone.clone(), mp3_data.clone()).await
                    {
                        web_sys::console::log_1(&format!("❌ MP3 playback failed: {:?}", e).into());
                        web_sys::console::log_1(
                            &format!(
                                "📊 MP3 data size: {} bytes, first bytes: {:02X?}",
                                mp3_data.len(),
                                &mp3_data[0..32.min(mp3_data.len())]
                            )
                            .into(),
                        );
                        {
    ...
    fn start_sound_pcm_fallback(self_rc: Rc<RefCell<Self>>, member_ref: DatumRef) {
        ...
        if let Some(sound_member) = Self::resolve_sound_member(player, &datum) {
            ...
            source
                ...
                .unwrap_or_else(|e| {
                    web_sys::console::log_1(
                        &format!("⚠️ Failed to add ended listener: {:?}", e).into(),
                    );
                });
            closure.forget();
    ...
    pub fn load_director_sound_from_bytes(
        ...
        let data_likely_pcm =
            expected_pcm_size.map_or(false, |expected| data.len() >= expected * 4 / 5);
        let is_mp3 = if data_likely_pcm {
            false
        } else {
            Self::find_mp3_start(data).is_some()
        };
    ...
    pub fn load_director_audio_data(
        ...
        let expected_pcm_size = expected_samples
            ...
            .map(|s| s as usize * channels as usize * bytes_per_sample_est);
        let data_likely_pcm =
            expected_pcm_size.map_or(false, |expected| sound_bytes.len() >= expected * 4 / 5);
    ...
    pub fn start_next_segment(&mut self) {
        ...
        if self.current_segment_index.is_none() {
            ...
            if self.loop_count == 0 {
                ...
                console::log_1(
                    &format!("🔁 Looping: {} loops remaining", self.loops_remaining).into(),
                );
