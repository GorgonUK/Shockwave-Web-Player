...
use super::cast_member::{
    ...
    film_loop::FilmLoopMemberHandlers, font::FontMemberHandlers, palette::PaletteMemberHandlers,
    sound::SoundMemberHandlers, text::TextMemberHandlers,
};
...
impl CastMemberRefHandlers {
    ...
    pub fn call(
        ...
        match handler_name.as_str() {
            ...
            "charPosToLoc" => reserve_player_mut(|player| {
    ...
    fn get_member_type_prop(
        ...
        match &member_type {
            ...
            CastMemberTypeId::Button => {
                ButtonMemberHandlers::get_prop(player, cast_member_ref, prop)
            }
                ...
                if prop == "text" {
                    ...
                    if let CastMemberType::Script(script_data) = &cast_member.member_type {
                        ...
                        web_sys::console::log_1(
                            &format!(
                                "⚠️ Trying to get .text from Script member #{}",
                                cast_member.number
                            )
                            .into(),
                        );
            ...
            CastMemberTypeId::VectorShape => {
                ...
                if let CastMemberType::VectorShape(vs) = &cast_member.member_type {
                    ...
                    if prop == "rect" {
                        ...
                        let vert_data: Vec<(i32, i32, i32, i32, i32, i32)> = vs
                            .vertices
                            .iter()
                            .map(|v| {
                                (
                            ...
                            .collect();
                        drop(cast_member);
                        let list: Vec<DatumRef> = vert_data
                            .iter()
                            .map(|(vx, vy, h1x, h1y, h2x, h2y)| {
                                let vertex_key =
                                    player.alloc_datum(Datum::Symbol("vertex".to_string()));
                                ...
                                let h1_key =
                                    player.alloc_datum(Datum::Symbol("handle1".to_string()));
                                ...
                                let h2_key =
                                    player.alloc_datum(Datum::Symbol("handle2".to_string()));
