use crate::{
    ...
    player::{
        ...
        font::{
            bitmap_font_copy_char, bitmap_font_copy_char_scaled, get_text_index_at_pos,
            measure_text, BitmapFont, DrawTextParams,
        },
            ...
            for cx in 0..canvas_width as i32 {
                ...
                if a > 0 {
                    if a == 255 || bitmap.data[dest_idx + 3] == 0 {
                        ...
                        bitmap.data[dest_idx + 1] =
                            ((g as u32 * alpha + bg_g * inv_alpha) / 255) as u8;
                        bitmap.data[dest_idx + 2] =
                            ((b as u32 * alpha + bg_b * inv_alpha) / 255) as u8;
    ...
    pub fn render_html_text_to_bitmap_styled(
        ...
        for line in &lines {
            ...
            for ch in line.chars() {
                ...
                let char_advance = ((font.get_char_advance(ch as u8) as i32) * requested_font_size
                    / native_char_height)
                    .max(1);
